...
class ProfileDetailAPIView(generics.RetrieveAPIView):
    ...
    def get_queryset(self) -> QuerySet:
        # returns the profile along with user data from the user model in the same query
...
class ProfileUpdateAPIView(generics.RetrieveUpdateAPIView):
    """
     handling both retrieval of a single object and updating that object. It provides built-in methods for fetching the object (via get_object) and handling the update (via perform_update).
    """
    ...
    def get_queryset(self):
        """
         Returning Profile.objects.none() means that no queryset is available for listing, but you are still retrieving a specific object via get_object(). This is valid, but typically get_queryset would be used to define a set of objects that can be listed.
        """
...
class NonTenantProfileListAPIView(generics.ListAPIView):
    ...
    filter_backends = [DjangoFilterBackend, filters.SearchFilter]
    #  user is the foreign key in the Profile model
    search_fields = ["user__username", "user__first_name", "user__last_name"]
    filterset_fields = ["occupation", "gender", "country_of_origin"]
